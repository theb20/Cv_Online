...
export default function FloatingLines({
  ...
  const getDist = (wave) => {
    ...
    const idx = enabledWaves.indexOf(wave);
    return idx >= 0 ? (lineDistance[idx] ?? 5) : 0.1;
  };

  const topCount    = enabledWaves.includes('top')    ? getCount('top')    : 0;
  const middleCount = enabledWaves.includes('middle') ? getCount('middle') : 0;
  const bottomCount = enabledWaves.includes('bottom') ? getCount('bottom') : 0;
  const topDist    = getDist('top')    * 0.01;
  const middleDist = getDist('middle') * 0.01;
  const bottomDist = getDist('bottom') * 0.01;
  ...
  useEffect(() => {
    ...
    const onPointerMove = (e) => {
      ...
      // Ignore si la souris est hors du canvas
    ...
    if (interactive) {
      // Écoute sur window pour capter la souris même quand elle est sur les éléments au-dessus
